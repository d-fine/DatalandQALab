...
def run_report_on_qalab(data_id: str, ai_model: str, use_ocr: bool) -> dict:
    ...
    params = {"ai_model": ai_model, "use_ocr": use_ocr, "force_override": False}
    response = requests.get(url, params=params, timeout=60 * 5)  # 5 minutes timeout since reviews can take time
    response.raise_for_status()
