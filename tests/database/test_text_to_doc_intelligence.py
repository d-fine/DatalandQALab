...
def test_extract_text_of_pdf(mock_credential: MagicMock, mock_client: MagicMock) -> None:
    ...
    result = old_extract_text_of_pdf(pdf_bytes)
...
def test_get_markdown_from_dataset_new_entry(
    ...
    data_id = "test_id"
    pdf_stream = io.BytesIO(dummy_pdf)
    pages = [1, 2, 3]
    ...
    mock_extract_text.assert_called_once_with(pdf_stream)
...
def test_get_markdown_from_dataset_existing_entry(
    ...
    mock_existing_entry.page_numbers = [1, 2]
